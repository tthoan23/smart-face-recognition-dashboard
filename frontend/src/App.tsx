...
export default function App() {
    ...
    const loadData = async () => {
      try {
        ...
        if (Array.isArray(personsData)) {
          setPersonsCount(personsData.length);
        }
      } catch (err) {
  ...
  const handleDeleteLog = async (logId: number) => {
    if (!window.confirm("Bạn có chắc muốn xóa bản ghi nhật ký này?")) {
      return;
    }
    ...
    try {
      const res = await fetch(`${API_URL}/api/events/${logId}`, {
        method: "DELETE",
      });
